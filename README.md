...
- **Разделение сервисов**: Разделяет сервисы для доступа к данным. 
...
`metrics_host_file_path` объявлена и не пустая.
Задает полный путь до файла куда будут записываться метрики с локального хоста.
`metrics_aggregate_delegate_host` объявлена и не пустая.
Задает имя хоста из инвентори, который будет агрегировать локальные метрики с локального хоста.
 - **Local Sender**: Часть роли `Local Scanner` в которой переменная `metrics_aggregate_delegate_host` объявлена и не пустая.
Передает локальные метрики на хост с ролью `Aggregate Exporter`.
 - **Local Exporter**: Часть роли `Local Scanner` в которой переменная `metrics_host_file_path` объявлена и не пустая.
Передает локальные метрики в локальный файл заданный в `metrics_host_file_path`.
 - **Aggregate Exporter**: Забирает локальные метрики с хостов, агрегирует их в локальный список агрегации и записывает их в локальный файл заданный в `metrics_aggregate_file_path`.
